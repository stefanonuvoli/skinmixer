        ...
        const nvl::Index& nodeId)
{
    ...
    Node& node = skinMixerGraph.node(nodeId);

    if (skinMixerGraph.node(nodeId).parents.empty()) {
        return;
    }
    ...
    for (VertexId vId = 0; vId < mesh.nextVertexId(); ++vId) {
        ...
        isNewVertex[vId] = findVertexBirthNode(skinMixerGraph, nodeId, vId, vertexBirthNode[vId], birthVertex[vId]);

        if (!isNewVertex[vId]) {
            assert(vertexBirthNode[vId] != nvl::MAX_ID);

            if (nodeGridMap.find(vertexBirthNode[vId]) == nodeGridMap.end()) {
                Model* nodeModel = skinMixerGraph.node(vertexBirthNode[vId]).model;
    ...
    for (JointId jId = 0; jId < skeleton.jointNumber(); ++jId) {
        isNewJoint[jId] = findJointBirthNode(skinMixerGraph, nodeId, jId, jointBirthNode[jId], birthJoint[jId]);
    }
    ...
    for (VertexId vId = 0; vId < mesh.nextVertexId(); ++vId) {
        ...
        for (JointId jId = 0; jId < skeleton.jointNumber(); ++jId) {
            if (jointBirthNode[jId] == bestRootNode) {
                ...
                const SkinningWeights& rootSkinningWeights = skinMixerGraph.node(bestRootNode).model->skinningWeights;
